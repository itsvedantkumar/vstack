...
- [x] R5 — Connectors: cloudflare-mcp, context7, namecheap OK; spaceship-mcp wired into vedant.to project (was orphaned); claude-mem repaired (npx claude-mem repair → doctor 5/5 green); claude-in-chrome extension-based (no file config, working at runtime)
...
- [x] R7 — gh authed; wrangler installed + authed via CLOUDFLARE_API_TOKEN; vercel installed (login = residual); verify gate ARMED on vedant-to-gh (.claude/verify.sh: typecheck+26 tests, falsifiability proven 0→1→0); scheduled routines: PRIMARY lane is claude.ai cloud routines (user's 2026-08-10 decision, 3 live triggers); local fallback repaired — repo-health SKILL.md rewritten gh-based (PAT-scraping removed) and live-tested via claude-task.sh (exit 0, real report in 30s); launchd plists built, validated, then parked at ~/.config/agents/backups/launchd-parked/ to avoid duplicating cloud runs
...
2. 1-min check at https://claude.ai/code/routines that the 3 cloud triggers are still active (their runs leave no local trace; local audit can't see them). If they're gone, re-arm locally: `launchctl bootstrap gui/$(id -u) ~/.config/agents/backups/launchd-parked/<plist>`.
