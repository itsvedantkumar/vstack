...
- **`leftbook` and `bb-1` were not audited.** Not reachable: no `~/.ssh/config`, no Tailscale, `known_hosts` contains only `github.com`. I used GitHub as a proxy and found **no cross-machine evidence** — all 951 human commits carry `+0530`; the only `+0000` commits are 5 GitHub web-editor edits. Three git identities exist but are per-repo config on this machine. If those machines carry materially different work, this ranking does not reflect it.
...
- Whether the exposed API tokens were ever rotated is **not measurable** from disk.
