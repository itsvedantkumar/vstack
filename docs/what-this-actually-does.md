...
|---|---|---|
| `register-warns` The register strike warns instead of blocking Stop (`claude/hooks/skill-mandate.sh`, 1.72.0). | At 1.71.0 the strike blocked Stop and Haiku 4.5 answered it with an extra turn in 10 of 30 runs; wall 1.67x gstack on multi_module (`tests/evals/showcase/runs/20260904-010328.38498.jsonl`). At 1.72.0 it warns: same fixture and model, vstack over gstack wall 1.05x, cost 1.04x, 10 of 10 green in both arms (`tests/evals/showcase/runs/20260904-121312.60002.jsonl`). | Run 2026-09-04 |
| `whitebox-mandate` The Stop gate refuses to let a session end without an audit when it was handed security work or changed a file whose path decides access (`claude/hooks/skill-mandate.sh`, 1.74.1). | On `traps/vuln_hunt`, 24 paired runs per arm on Haiku 4.5: vstack left 0.96 of 4 planted vulnerabilities open against gstack's 2.75 and pstack's 2.50, p < 0.00001 both ways (200,000-relabeling permutation test), and the `whitebox-pentest` skill fired in 24 of 24 runs. At 1.74.0 the same fixture had vstack last of three at 2.80 with the skill firing in 0 of 15. It costs 3.8x gstack's spend, and 14 of 24 vstack runs still said DONE on a red held-out check (`tests/evals/showcase/runs/20260905-231356.97020.jsonl`, `tests/evals/showcase/runs/20260905-221043.79057.jsonl`). | Run 2026-09-05 |
...
| `breadth-retired` Delegation-breadth mandate, retired (`claude/hooks/skill-mandate.sh:730-741`) | No longer blocks. Formerly forced fan-out on `dir_count >= 2 && ext_count >= 2 && fanout_batches == 0`; retired in 1.68.0 (commit `773f3d4`) after `tests/evals/showcase/runs/20260903-022953.55428.jsonl` showed it fired on 2 of 5 headless runs of the same fix at 3.6x-4.4x the cost of the 3 runs where it stayed silent, with no correctness gain. | Nothing outstanding; this row is the closed measurement, kept here rather than section 1 because it documents a mechanism (now inert) plus its own removal rationale. |
...
| `per-prompt-digest` Per-prompt digest (`claude/hooks/inject-session-context.sh`) | Injects at most two conditional lines per `UserPromptSubmit`: `GRILL:` past 320 prompt characters and a mandate strike when a Stop-hook counter file exists; nothing otherwise. The unconditional TOKENS/DELEGATE/FANOUT lines were cut in 1.71.0 (vstack spawned 0 of 10 with them present, RESULTS.md). Check 18 proves 0 B unfired and 215 B fired under a 512 B cap. | Effect of the two remaining lines on any outcome. The 9/9 vs 7/9 ablation in section 1 is about the SessionStart routing table in the same script. Unmeasured; decide by 2026-09-17. |
| `skill-load-ledger` Skill-load replay rows (`claude/hooks/dispatch-counter.sh`, matcher `Agent|Task|Skill`) | Appends a `kind: skill_load` row with the skill name and `duration_ms` on every Skill PostToolUse; `bin/doctor --ledger` sums per session. Check 44 proves the matcher names Skill in both `claude/settings.json` and `install.sh` (row 44i). | Whether any decision uses the sums; the join to benchmark rows needs `session_id` in `run.sh`, not yet added. Unmeasured; decide by 2026-09-17. |
