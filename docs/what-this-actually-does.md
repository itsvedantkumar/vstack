...
|---|---|---|
| The gate is 43 checks, all currently green. | `./.claude/verify.sh` prints `checks: 43 declared, 43 ran, 0 skipped` and `VERIFIED`, run against this tree at v1.40.0. | Run 2026-08-23 |
| Every one of those 43 checks has a falsifiability row that has been watched going red. `tests/gate-falsifiability.sh` breaks exactly what each check watches, requires the gate to name it, and restores the file byte for byte. Check 16 fails the gate if a check has no such row. | `tests/gate-falsifiability.sh`, `tests/README.md`, README.md "Checks that can fail" | Mechanism as of v1.40.0. Suite not re-run for this document; row 40 verified by the scoped-row method only, see note below |
...
This is the closest thing this repository has to a stress test of the "43 checks, all
...
completion when the repository's own gate is red, and a gate of 43 checks with a documented, if
