...
| The destructive-command guard decides correctly on a fixed set of 30 commands across 3 tiers (deny, ask, allow), including under a stripped environment (`-u TMPDIR -u HOME -u USER -u LANG`) and malformed or empty input. | `.claude/verify.sh` check 23, run 2026-08-23, currently `ok` | 2026-08-23 |
