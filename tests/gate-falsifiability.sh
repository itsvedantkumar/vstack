...
files_for(){ case "$1" in
  ...
  66d) printf 'claude/whitebox-audit.sh' ;;
  66e) printf 'claude/whitebox-audit.sh' ;;
...
label_for(){ case "$1" in
  ...
  66d) printf "the deep security lane refuses to report clean when nothing scanned" ;;
  66e) printf "the deep security lane refuses to report clean when nothing scanned" ;;
...
' install.sh && rm -f install.sh.t ;;
  ...
  66) # Turn the "nothing ran" refusal into a pass. This is the mutation that matters: a deep
      ...
      sed -i.t 's|^if \[ "$produced" -eq 0 \]; then|if [ "$produced" -lt 0 ]; then|' \
  ...
  66d) # Make the abort detector answer "this tool ran fine" for everything. A scanner that died
      # on a failed database download exits with the same code it uses for a CVE, so with this
      # mutation its failure is filed as a security finding and the errored count stays at zero:
      # coverage lost, reported as work done. Mutating the decision itself, not the signature
      # list, because a row pinned to one signature stops mutating anything the day that list is
      # rewritten -- and reports the check unfalsifiable while proving nothing.
      sed -i.t 's|^aborted() { # <raw_output_file>|aborted() { return 1 # <raw_output_file>|' \
        claude/whitebox-audit.sh && rm -f claude/whitebox-audit.sh.t ;;

  66e) # The opposite direction: every non-zero exit becomes "could not run". A lane that calls
      # everything an error passes 66d and reports nothing, ever. Without this row, the fix for
      # 66d could be a function that always returns true.
      sed -i.t 's|^aborted() { # <raw_output_file>|aborted() { return 0 # <raw_output_file>|' \
        claude/whitebox-audit.sh && rm -f claude/whitebox-audit.sh.t ;;
  ...
  12b) # Row 12 breaks a README TABLE cell, so it proves only check 12's table extractor. The
      # prose extractor, want_for's noun map and the count each noun resolves to are a separate
      # lane, and the "falsifiability rows" noun was added to close a claim that had been wrong
      # for four releases while the table lane stayed green. Falsify the prose claim instead.
  ...
  62) # Widen the exemption. One more entry is how a carve-out written for one measured incident
      # becomes a general amnesty, and the list is the only thing standing between the two.
      printf "PRETAG_ALLOWED_FINDING='doctor is green on a clean install'\n" \
        >> tests/pretag-findings.sh ;;

  62b) # The half that matters more, and the half a check on the list alone would miss: leave the
      # list at one entry and downgrade the finding inside bin/doctor instead. The two harnesses
      # go on passing either way; what changes is that every user's doctor stops reporting a
...
' claude/hooks/verify-gate.sh && rm -f claude/hooks/verify-gate.sh.t ;;
  ...
  # Row 27b lived here: it flipped the delegation breadth mandate's fanout_batches==0 condition
  # back onto task_count. 1.68.0 retired that mandate on measured cost (see the retirement comment
  # in claude/hooks/skill-mandate.sh, and tests/evals/showcase/RESULTS.md's routing-cost table),
  # so the sed anchor it mutated no longer exists in the file -- and a row whose anchor is gone
  # mutates nothing, then reports "did NOT fail when broken", which reads exactly like a check
  # that cannot be broken. Deleted with the rule, id retired rather than reused. 27 and 27c still
  # cover check 27 (unconditional block; unreachable swarm mandate).
