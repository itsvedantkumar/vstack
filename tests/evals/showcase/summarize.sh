...
| jq -s --argjson idx "$idx" --arg commit "$(git rev-parse --short HEAD)" '
  ...
  [ .[] | .file as $f
    ...
    | select(.arm == "vstack" or .arm == "gstack" or .arm == "pstack")
    | . + { engine: $idx[$f].engine,
  ...
  | { generated: (now | todate), commit: $commit,
      ...
      # "vstack vs gstack": the paired runs where every harness saw the same fixture, model and
      # vstack version (three-arm notes read "vstack vs gstack vs pstack" and match too). Never
      # pooled across files. The rule from Vedant (2026-09-04): the comparison is vstack against
      # gstack and pstack; rows from any other arm are excluded everywhere.
