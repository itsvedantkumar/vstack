...
const MECHANISMS = [
  ["what the harness adds", "bare Claude Code", "gstack (0d1bd56)", "vstack (v1.71.0)"],
  ["hooks wired", "none", "SessionStart only", "SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, PostToolUseFailure, Stop"],
  ["stop gate", "none", "none", "Stop hook runs the project's verify.sh; a red gate blocks the turn"],
  ["gate self-test", "n/a", "none", "66 checks, 118 falsifiability rows (each check shown going red under its own mutation)"],
  ["context injected per session", "0 B", "45 KB CLAUDE.md (contributor file)", "about 3.2 KB full / 2.5 KB plugin"],
  ["skills shipped", "0", "54 top-level", "28"],
...
function verdict(h2h) {
  ...
  for (const [file, rows] of Object.entries(byFile)) {
    ...
    lines.push(`${rows[0].model}, ${rows[0].fixture}: ${rows.map((r) => `${r.arm} ${r.green}/${r.n} green, ${r.false_completion} false completions`).join("; ")} -> ${same ? "parity" : "DIFFERENT"}`);
  }
...
<title>vstack vs gstack vs bare Claude Code: measured</title>
...
<h1>vstack vs gstack vs bare Claude Code</h1>
...
<div class="v"><b>Verdict from the paired three-arm runs:</b> ${v.anyDiff ? "at least one paired run shows a difference between arms" : "parity. No arm beats another on held-out correctness or false completions in any paired run. vstack does not beat gstack, and gstack does not beat bare Claude Code, on these fixtures."}<br>
<small>${v.lines.map(esc).join("<br>")}</small></div>
<h2>Head to head: same fixture, same model, same day, all arms</h2>
<p>Arms: <b>none</b> is Claude Code with an empty project <code>.claude</code>; <b>gstack</b> and <b>vstack</b> are the two harnesses installed project-scoped (<code>--setting-sources=project</code>). "Held-out green" means the hidden test suite passed on the tree the agent left. "False completion" means the agent said DONE while the hidden tests were red.</p>
...
<p>Includes the OpenCode engine runs (GLM 5.3 Flash, free lane) with harness-side <b>gate</b> and <b>oracle</b> arms, and older single-harness runs. Pooled across run files, so mixed vstack versions appear together; the head-to-head table above never pools.</p>
