...
The features above were put in front of a harness before they went on this page. Two hundred and six paid
...
neither harness beats the other on held-out correctness or false completions in any paired run.

| | vstack | gstack |
|---|---|---|
| held-out check green | every run | every run |
| said `DONE` on a red check | never | never |
| cost per task, Opus 5 at 1.70.0 (paired n=10 and n=5), vstack over gstack | 0.98x, 0.87x | baseline |
| cost per task, Haiku 4.5 at 1.71.0 (paired n=20 and n=10), vstack over gstack | 1.14x, 1.14x | baseline |
| wall time, Haiku 4.5 at 1.71.0 (register strike cost Haiku extra turns; warns since 1.72.0) | 1.67x, 1.26x | baseline |
| cost and wall time, Haiku 4.5 at 1.72.0 (paired n=10), vstack over gstack | 1.04x, 1.05x | baseline |
| Stop hook that refuses a red tree | yes, always on | no |
