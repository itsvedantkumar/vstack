...
| `.claude/verify.sh` | **this repository's own gate**, 53 checks; not shipped to anyone |
...
```bash
curl -fsSL https://raw.githubusercontent.com/itsvedantkumar/vstack/v1.47.0/bootstrap.sh -o bootstrap.sh
VSTACK_REF=v1.47.0 bash bootstrap.sh     # installs that tag, not main
```
...
The gate is 53 checks (this number moves as checks are added; check 12 fails if this prose
