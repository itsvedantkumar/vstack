...
```bash
curl -fsSL https://raw.githubusercontent.com/itsvedantkumar/vstack/v1.64.0/bootstrap.sh -o bootstrap.sh
VSTACK_REF=v1.64.0 bash bootstrap.sh     # installs that tag, not main
```
...
The gate is 63 checks (this number moves as checks are added; check 12 fails if this prose
...
At 107 falsifiability rows and a ~84s gate, that is over two hours serially. This paragraph claimed
