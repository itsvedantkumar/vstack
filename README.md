...
The features above were put in front of a harness before they went on this page. Four hundred and forty-two paid
runs on Opus 5 and Haiku 4.5, vstack against gstack and against pstack (the Claude Code port of
Cursor's pstack), seven fixtures with held-out checks the agent never sees.
...
that file from this branch and links every row to the run file behind it). On correctness every
arm ties: no harness beats another on held-out checks or false completions in any paired run.
On Opus 5 vstack is the fastest of the three, at a cost within noise of gstack's. On the one
fixture that ships its own gate and does not mention it, pstack said `DONE` on a red tree once in
ten while vstack and gstack did not; pstack was also the cheapest arm there, so read the two rows
together. On `gate_bites`, the fixture built so that fixing the reported symptom leaves the
...
|---|---|---|---|
| held-out check green | every run | every run | 9 of 10 on the gated fixture |
| said `DONE` on a red check | never | never | once in ten on the gated fixture |
