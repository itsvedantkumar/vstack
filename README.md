...
| Checks proving the thing works | none found | 41 checks |
| Mutations proving those checks can fail | none found | at least one per check, enforced by check 16 |
...
`.claude/verify.sh` runs 41 checks. `tests/gate-falsifiability.sh` breaks the repository at least
...
The v1.18.0 audit found thirteen more, and they fall into four shapes worth naming:
...
```bash
curl -fsSL https://raw.githubusercontent.com/itsvedantkumar/vstack/v1.18.0/bootstrap.sh -o bootstrap.sh
less bootstrap.sh                        # about 100 lines
VSTACK_REF=v1.18.0 bash bootstrap.sh     # installs that tag, not main
```
